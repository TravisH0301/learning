...
- [Data Vault Components](#data-vault-components)
  ...
  - [Link](#link)
  - [Satellite](#satellite)
- [Comparison in data models](#comparison-in-data-models)

A Data Vault is a data modeling methodology and architecture designed for enterprise data warehousing that emphasizes flexibility, scalability and historical tracking.
It is a hybrid approach that combines the structure of a 3rd Normalised Form (3NF) model with the query efficiency concepts of a Star Schema, making it well-suited for integrating large volumes of data from diverse source systems, especially in environments with rapidly changing business requirements.

At any enterprise data landscape, data vault may be implemented as part of a medallion architecture (or Multi-hop) to progressviely develop structure and quality of the data assets.
...
### Link
Link tables (also defined as transaction, hierarchy or relationship) act as a **linkage**, defining relationships between two or more hubs. The link contains the primary keys of the connecting hubs as the foreign keys. And the link's 
...
### Stand-alone tables
Stand-alone tables (=Reference tables) such as calendars, time, code and description look-ups may be utilised.

## Comparison in data models
Feature|Data Vault (DV)|Third Normal Form (3NF)|Dimensional Model (Star Schema)
|--|--|--|--|
Primary Goal|Integration| Auditability| Flexibility|Data Integrity| Minimal Redundancy|Query Performance| Usability
Handling Change|Excellent. Highly flexible. New attributes/sources only require adding new Satellites.|Poor. Schema changes are complex and often require cascading updates.|Fair/Poor. Changes to business rules often require significant redesign.
Data History|Full History. Insert-only approach preserves all data for maximum auditability.|Good Lineage. Tracks history via SCD or complex auditing.|Good History. Handled via Slowly Changing Dimensions (SCDs).
Complexity|High. Requires specialized knowledge (Hubs, Links, Satellites).|Medium/High. Requires deep knowledge of normalization rules.|Low. Simple, intuitive structure (facts surrounded by dimensions).
Query Performance|Lower. Not optimized for end-user reporting; requires complex joins.|Lower. High normalization results in many joins, leading to slower read performance.|High. Optimized for read operations and BI tools due to denormalization.
Storage|Higher. Highly normalized, insert-only structure creates many tables and some key redundancy.|Lowest. Primary objective is minimal redundancy.|Medium/Higher. Denormalization leads to data redundancy in dimension tables.
Layer Position|Integration Layer (Core Data Warehouse)|Integration Layer (Core Data Warehouse)|Presentation Layer (Data Marts/BI Layer)
